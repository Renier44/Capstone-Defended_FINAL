...
def patient_details(request, id):
    """
    JSON Endpoint for the 'View Details' modal.
    Strictly sends 'is_ai_screening' so the frontend can adjust the label.
    """
    try:
        ...
        data = {
            ...
            "doctor": patient.doctor.user.get_full_name() if patient.doctor else "Unassigned",
            
            # Format date clearly
            "appointment_datetime": patient.appointment_datetime.strftime("%B %d, %Y %I:%M %p"),
            "status": patient.status,

            # [PROFESSOR FIX] STRICT DATA PASSING
            # 1. Pass the flag
            "is_ai_screening": patient.is_ai_screening, 
            
            # 2. Pass the result (if AI)
            "preliminary_result": patient.preliminary_result if patient.preliminary_result else "Pending",
            
            # 3. Pass the reason (if Standard)
            "reason": patient.reason if patient.reason else "None",
        }
...
class UpdateProfileAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # 1. Update Django User fields
        user.first_name = data.get('first_name', user.first_name)
        user.last_name = data.get('last_name', user.last_name)
        
        # Handle email change and username (Keep this logic)
        ...
        # 2. Update UserProfile fields (Age/DOB and Gender)
        # Assuming your UserProfile model is linked to User via a related_name='userprofile' 
        # or accessed directly via user.userprofile or user.profile (change 'userprofile' if needed)
        try:
            # FIX: Access the UserProfile using its related name or attribute
            profile = user.userprofile  # OR user.profile OR UserProfile.objects.get(user=user)
        except UserProfile.DoesNotExist:
             return Response({"error": "User profile not found."}, status=status.HTTP_404_NOT_FOUND)

        # Update profile data received from the app
        profile.date_of_birth = data.get('date_of_birth', profile.date_of_birth)
        # Note: We save gender in lowercase, as established in your RegisterSerializer
        gender_data = data.get('gender', profile.gender)
        if gender_data:
            profile.gender = gender_data.lower()
            
        # Remove any unnecessary fields like 'age' if you are calculating it
        # profile.age = None # Don't update age here

        profile.save()
        
        # 3. Prepare Response (Include the updated profile data)
        
        # Handle Doctor-specific fields if they exist, otherwise default to None
        # You might need to adjust this depending on whether a patient has a Doctor profile
        phone = getattr(user.doctor, 'phone', None) if hasattr(user, 'doctor') else None
        address = getattr(user.doctor, 'address', None) if hasattr(user, 'doctor') else None
        profile_image = getattr(user.doctor, 'profile_image', None) if hasattr(user, 'doctor') else None
        ...
        return Response({
            ...
            "last_name": user.last_name,
            "email": user.email,
            # ✅ FIX: Return date_of_birth and gender from the profile
            "date_of_birth": profile.date_of_birth,
            "gender": profile.gender, 
            
            # Optionally include Doctor/related fields
            "phone": phone,
            "address": address,
...
from django.contrib.auth.models import User
from .models import AdminProfile  # Separate profile for admins
...
def admin_profile_view(request):
    """Renders the admin profile page with user and admin profile data."""
    profile = request.user.adminprofile  # Use AdminProfile
    context = {
        'user': request.user,
        'profile': profile,
    }
...
def admin_update_profile(request):
    """Handles updating the admin's profile."""
    if request.method == 'POST':
        user = request.user
        # Get or create related admin profile
...
@login_required
@user_passes_test(is_admin, login_url='/login/')
def admin_update_profile_image(request):
    """Handles updating the admin's profile image."""
    if request.method == 'POST':
